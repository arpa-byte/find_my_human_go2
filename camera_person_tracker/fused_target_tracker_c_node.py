...
class FusedTargetTrackerCNode(Node):
    def __init__(self) -> None:
        super().__init__("fused_target_tracker_c")
        ...
        self.declare_parameter(
            "fused_marker_topic",
            "/camera_person_tracker/fused_target_marker_c",
        )
        self.declare_parameter("fused_target_frame", "fused_target_c")
        ...
        self.fused_marker_pub = self.create_publisher(
            Marker,
            self.fused_marker_topic,
            10,
        )
        self.timer = self.create_timer(self.publish_period_sec, self.update)

        self.get_logger().info("FusedTargetTrackerCNode started.")
    ...
    def publish_fused_marker(
        ...
        marker.text = (
            f"{state} | lidar_id="
            f"{locked_track_id if locked_track_id is not None else 'None'}"
        )
    ...
    def maybe_log_match_debug(
        ...
        if camera_obs is None:
            self.get_logger().info("[FusionC DEBUG] camera_target=None")
        else:
            cam_state = "ACTIVE" if camera_obs.visible else (
                "REMEMBERED" if camera_obs.remembered else "UNKNOWN"
            )
            self.get_logger().info(
                f"[FusionC DEBUG] camera_target state={cam_state} "
                f"pos=({camera_obs.x:.2f}, {camera_obs.y:.2f}, {camera_obs.z:.2f})"
            )

        if not lidar_tracks:
            self.get_logger().info("[FusionC DEBUG] lidar_tracks=None")
            return

        sorted_tracks = sorted(lidar_tracks.values(), key=lambda t: t.track_id)
        for track in sorted_tracks:
            self.get_logger().info(
                f"[FusionC DEBUG] lidar_track id={track.track_id} "
        ...
        if camera_obs is not None:
            ...
            if best_track_id is not None:
                ...
                self.get_logger().info(
                    f"[FusionC DEBUG] nearest_lidar_to_camera "
                    f"id={best_track_id} "
