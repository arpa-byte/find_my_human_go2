...
class PointCloudSliceToScanNode(Node):
    """
    ...
    Intended for converting Livox MID360 point clouds into a pseudo-2D scan
    that can later be fed into 2D laser-based human detection pipelines.
    """

    def __init__(self) -> None:
        super().__init__("pointcloud_slice_to_scan")
        ...
        # Only points with z in [slice_z_min, slice_z_max] are kept.
        self.declare_parameter("slice_z_min", -0.60)
        self.declare_parameter("slice_z_max", -0.40)
        ...
        self.get_logger().info("PointCloudSliceToScanNode started.")
...
def main(args=None) -> None:
    ...
    node = PointCloudSliceToScanNode()
    try:
        rclpy.spin(node)
